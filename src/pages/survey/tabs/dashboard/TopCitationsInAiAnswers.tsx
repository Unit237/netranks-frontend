...
export default function TopCitationsInAiAnswers({ survey }) {
  const defaultLimit = 5;

  const citations = survey.TopCitationsInAiAnswers;
      ...
      <CardContent>
        ...
        <Grid container spacing={2} columns={12} sx={{ mb: (theme) => theme.spacing(2), maxHeight: 350, overflowY: "scroll" }}>
          <Grid size={{ xs: 12 }} style={{ paddingTop: 32 }}>
            {citations.slice(0, showAll ? citations.length : defaultLimit).map((x, i) => (
              <Stack
                key={i}
                direction="row"
